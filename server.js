...
io.on('connection', (socket) => {
  ...
  socket.on('creer_partie', () => {
    ...
    socket.emit('partie_creee', { code: partie.code, mjToken: partie.mjToken });
  });

  socket.on('mj_reconnecter', ({ roomCode, mjToken }) => {
    const partie = parties.get(roomCode);
    if (!partie || partie.mjToken !== mjToken) return socket.emit('erreur', { message: 'Impossible de reprendre cette partie.' });
    partie.mjSocketId = socket.id;
    socket.join(partie.code);
    socket.data.roomCode = partie.code;
    socket.data.estMJ = true;
    socket.emit('partie_creee', { code: partie.code, mjToken: partie.mjToken });
  ...
  socket.on('rejoindre_partie', ({ roomCode, prenom, cartePhysique, avatarData }) => {
    const partie = parties.get(roomCode);
    ...
    const joueur = G.creerJoueur(socket.id, prenom, cartePhysique, avatarData);
  ...
  socket.on('reconnecter', ({ roomCode, playerId }) => {
    ...
    if (joueur.statut === 'mort' || joueur.cartePhysique === 'medium') socket.join(`${partie.code}:cimetiere`);
  ...
  socket.on('mj_lancer_phase', ({ phase, duree }) => {
    ...
    if (phase === 'nuit') {
      ...
      partie.joueurs.forEach((j) => {
        ...
        if (j.signalPerduActif) {
          j.signalPerduActif = false;
          io.to(j.socketId).emit('signal_perdu', { actif: false });
        }
      });
      partie.state = 'nuit';
  ...
  socket.on('mj_attribuer_maire', ({ playerId }) => {
    const partie = parties.get(socket.data.roomCode);
    if (!estMJ(socket, partie)) return;
    const cible = partie.joueurs.get(playerId);
    if (!cible) return;

    if (cible.estMaire) {
      // Le MJ reclique sur le Maire actuel : on retire le badge, personne n'est Maire.
      cible.estMaire = false;
    } else {
      // Nouveau Maire : on retire le badge à tout le monde d'abord (un seul Maire actif à la fois).
      partie.joueurs.forEach((j) => { j.estMaire = false; });
      cible.estMaire = true;
    }
    diffuserEtat(partie);
  });
  ...
  socket.on('mj_resoudre_egalite', ({ playerId }) => {
    ...
    partie.state = 'jour';
    io.to(partie.code).emit('phase_changee', { phase: 'jour', jourIndex: partie.jourIndex, gazetteEvenement: partie.gazetteEvenement });
